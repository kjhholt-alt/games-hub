// Combine six cube Sealed seed-shards (mtg-forge-shard's cube-receipts-hardened,
// cube-receipts-s4, cube-receipts-s23..s26 -- the six shards on the node that
// finished with every one of their 45 receipts rules_engine_verified, no holds,
// no unreproduced runs) into ONE receipted standings payload: 270 real
// Card-Forge Sealed matches (45 pairings x 6 seeds) across the ten guild Cube
// shells, summed per deck, with a per-match record carrying its own
// receipt_hash so the league page can link every score to the raw match it
// came from.
//
// This is additive evidence, not a replacement:
// public/mtg-proving-grounds-cube-standings.json (single shard, seed 20260730,
// status "provisional") is untouched. This script only ever WRITES
// public/mtg-proving-grounds-cube-sealed-combined.json.
//
// Same fail-closed contract as scripts/build-forge-combined-standings.mjs
// (the Brawl analog this was forked from): throws and writes nothing if any
// shard directory is missing, has other than 45 receipt files, contains a
// receipt that isn't rules_engine_verified with empty held_reasons and a
// proven two-run repeat, or if a match's winner label can't be resolved to
// exactly one of its two deck_files entries. Status is only ever "receipted"
// because promoted == planned is verified before writing.
//
// Usage: node scripts/build-cube-sealed-combined.mjs <mtg-forge-shard root>
...
const OUT_PATH = path.join(ROOT, "public", "mtg-proving-grounds-cube-sealed-combined.json");

const SHARD_DIRS = [
  "cube-receipts-hardened",
  "cube-receipts-s4",
  "cube-receipts-s23",
  "cube-receipts-s24",
  "cube-receipts-s25",
...
function fail(message) {
  throw new Error(`build-cube-sealed-combined: ${message}`);
}
...
function main() {
  ...
  if (!forgeShardRoot) {
    fail("usage: build-cube-sealed-combined.mjs <mtg-forge-shard root>");
  }
  ...
  for (const shardDir of SHARD_DIRS) {
    ...
    for (const file of files) {
      ...
      if (receipt.game_format !== "Sealed") {
        fail(`${shardDir}/${file} game_format is "${receipt.game_format}", not Sealed -- stopping`);
      }
      if (jarSha256 === null) {
        jarSha256 = receipt.engine.jar_sha256;
        engineVersion = receipt.engine.version;
      ...
      // Each match in a shard gets its own auto-incremented seed -- record the
      // shard's base seed from the alphabetically-first match, matching how
      // the plan files label a shard by its first match's seed.
      if (shardSeed === null) {
  ...
  const payloadWithoutHash = {
    schema: "buildkit-mtg-cube-sealed-combined-standings@1",
    ...
    engine: { name: "Card-Forge", version: engineVersion, jar_sha256: jarSha256 },
    seed_sets: seedSets,
    ...
    limitations: [
      ...
      "This proves reproducible pinned-engine Sealed matchups repeated across six seeds, not a live metagame win rate.",
      "Deck AI quality, not just deck strength, influences these results.",
      "Only the ten Forge-compatible admitted Cube guild shells are exercised here; the synthetic 20-deck sample table on this page uses an unrelated fixture roster and is never mixed with this data.",
    ],
    table,
    matches: perMatch,
  };
